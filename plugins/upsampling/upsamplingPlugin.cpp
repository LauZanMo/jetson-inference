...
int NearestNeighborUpsamplingPlugin::enqueue(int                batchSize,
    ...
    if (mDataType == DataType::kFLOAT)
        CHECK(cudaResizeNearestNeighbor<float>((float *)inputs[0], mNbInputChannels, mInputHeight,
                                               mInputWidth, (float *)outputs[0], stream));
    else
        CHECK(cudaResizeNearestNeighbor<__half>((__half *)inputs[0], mNbInputChannels, mInputHeight,
                                                mInputWidth, (__half *)outputs[0], stream));
    return 0;
}

IPluginV2 *NearestNeighborUpsamplingPluginCreator::createPlugin(const char                  *name,
