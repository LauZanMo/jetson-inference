...
size_t DimsUtils::getTensorSize(Dims dims) {
    ...
    size_t res =
        std::accumulate(dims.d, dims.d + dims.nbDims, (size_t)1, std::multiplies<size_t>());
    // Dims.d is int, so overflow check.
    assert(res ==
           (size_t)std::accumulate(dims.d, dims.d + dims.nbDims, (int)1, std::multiplies<int>()));
    return res;
}
...
// -----------------------------------------------------------------
IPluginLayer *
addPlugin(INetworkDefinition &network, ITensor *const *inputs, int num_inputs, IPlugin *plugin) {
...
ILayer *addElu(IPluginContainer   &plugin_factory,
...
ILayer *addCostVolume(IPluginContainer   &plugin_factory,
                      INetworkDefinition &network,
                      ITensor            &left_input,
                      ITensor            &right_input,
                      CostVolumeType      cv_type,
                      int                 max_disparity,
                      DataType            data_type,
                      const std::string  &name) {
    // Create plugin.
...
ILayer *addConv3D(IPluginContainer   &plugin_factory,
                  INetworkDefinition &network,
                  ITensor            &input,
                  Conv3DType          conv_type,
                  Dims                kernel_dims,
                  Dims                stride_dims,
                  Dims                pad_start_dims,
                  Dims                pad_end_dims,
    ...
    // Create plugin.
    auto plugin =
        plugin_factory.createConv3DPlugin(conv_type, kernel_dims, stride_dims, pad_start_dims,
                                          pad_end_dims, kernel_weights, bias_weights, name);
    assert(plugin != nullptr);
    // Add to the network.
    ITensor *inputs[] = {&input};
    auto     layer    = addPlugin(network, inputs, 1, plugin);
...
ILayer *addConv3DTranspose(IPluginContainer   &plugin_factory,
                           INetworkDefinition &network,
                           ITensor            &input,
                           Conv3DType          conv_type,
                           Dims                kernel_dims,
                           ...
                           const std::string  &name) {
    // Create plugin.
    auto plugin = plugin_factory.createConv3DTransposePlugin(
        conv_type, kernel_dims, out_dims, stride_dims, pad_start_dims, pad_end_dims, kernel_weights,
        bias_weights, name);
    assert(plugin != nullptr);
    // Add to the network.
    ITensor *inputs[] = {&input};
...
ILayer *addSlice(IPluginContainer   &plugin_factory,
                 INetworkDefinition &network,
                 ITensor            &input,
                 Dims                dims,
                 Dims                slice_start,
...
ILayer *addTransform(IPluginContainer   &plugin_factory,
                     INetworkDefinition &network,
                     ITensor            &input,
...
ILayer *addPad(IPluginContainer   &plugin_factory,
               INetworkDefinition &network,
               ITensor            &input,
               DimsNCHW            pad_start,
               DimsNCHW            pad_end,
...
ILayer *addSoftargmax(IPluginContainer   &plugin_factory,
                      INetworkDefinition &network,
                      ITensor            &input,
...
void reportError(cudaError_t status, const char *file, int line, const char *func, ILogger &log) {
    if (status == cudaSuccess)
        return;
    std::ostringstream str;
    str << file << ":" << line << ": " << func << ": CUDA error " << status << " ("
...
void reportError(cudnnStatus_t status, const char *file, int line, const char *func, ILogger &log) {
    if (status == CUDNN_STATUS_SUCCESS)
        return;
    std::ostringstream str;
    str << file << ":" << line << ": " << func << ": cuDNN error " << status << " ("
...
IPlugin *StereoDnnPluginFactory::createPlugin(const char *layerName,
