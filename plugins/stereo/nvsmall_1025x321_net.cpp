...
INetworkDefinition *createNVSmall1025x321Network(IBuilder         &builder,
    ...
    auto left_conv2 =
        network->addConvolution(*left_conv1_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto left_conv2_act =
        addElu(plugin_factory, *network, *left_conv2->getOutput(0), data_type, "left_conv2_act");
    assert(left_conv2_act != nullptr);
    left_conv2_act->setName("left_conv2_act");

    // right_conv2 convolution op.
    auto right_conv2 =
        network->addConvolution(*right_conv1_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto right_conv2_act =
        addElu(plugin_factory, *network, *right_conv2->getOutput(0), data_type, "right_conv2_act");
    assert(right_conv2_act != nullptr);
    right_conv2_act->setName("right_conv2_act");

    // left_conv3 convolution op.
    auto left_conv3 =
        network->addConvolution(*left_conv2_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto left_conv3_act =
        addElu(plugin_factory, *network, *left_conv3->getOutput(0), data_type, "left_conv3_act");
    assert(left_conv3_act != nullptr);
    left_conv3_act->setName("left_conv3_act");

    // right_conv3 convolution op.
    auto right_conv3 =
        network->addConvolution(*right_conv2_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto right_conv3_act =
        addElu(plugin_factory, *network, *right_conv3->getOutput(0), data_type, "right_conv3_act");
    assert(right_conv3_act != nullptr);
    right_conv3_act->setName("right_conv3_act");

    // left_conv4 convolution op.
    auto left_conv4 =
        network->addConvolution(*left_conv3_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto left_conv4_act =
        addElu(plugin_factory, *network, *left_conv4->getOutput(0), data_type, "left_conv4_act");
    assert(left_conv4_act != nullptr);
    left_conv4_act->setName("left_conv4_act");

    // right_conv4 convolution op.
    auto right_conv4 =
        network->addConvolution(*right_conv3_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto right_conv4_act =
        addElu(plugin_factory, *network, *right_conv4->getOutput(0), data_type, "right_conv4_act");
    assert(right_conv4_act != nullptr);
    right_conv4_act->setName("right_conv4_act");

    // left_conv5 convolution op.
    auto left_conv5 =
        network->addConvolution(*left_conv4_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto right_conv5 =
        network->addConvolution(*right_conv4_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto cost_vol = addCostVolume(plugin_factory, *network, *left_conv5->getOutput(0),
                                  *right_conv5->getOutput(0), CostVolumeType::kDefault, 48,
                                  data_type, "cost_vol");
    ...
    auto conv3D_1 = addConv3D(plugin_factory, *network, *cost_vol->getOutput(0),
                              ...
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_1_k"),
                              weights.at("conv3D_1_b"), "conv3D_1");
    assert(conv3D_1 != nullptr);
    conv3D_1->setName("conv3D_1");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_1_tran = addTransform(plugin_factory, *network, *conv3D_1->getOutput(0),
                                      {1, 0, 2, 3}, "conv3D_1_tran_transform");
    assert(conv3D_1_tran != nullptr);
    conv3D_1_tran->setName("conv3D_1_tran");

    // conv3D_1_act ELU activation op.
    auto conv3D_1_act =
        addElu(plugin_factory, *network, *conv3D_1_tran->getOutput(0), data_type, "conv3D_1_act");
    ...
    auto conv3D_2 = addConv3D(plugin_factory, *network, *conv3D_1_act->getOutput(0),
                              ...
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_2_k"),
                              weights.at("conv3D_2_b"), "conv3D_2");
    assert(conv3D_2 != nullptr);
    conv3D_2->setName("conv3D_2");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_2_tran = addTransform(plugin_factory, *network, *conv3D_2->getOutput(0),
                                      {1, 0, 2, 3}, "conv3D_2_tran_transform");
    assert(conv3D_2_tran != nullptr);
    conv3D_2_tran->setName("conv3D_2_tran");

    // conv3D_2_act ELU activation op.
    auto conv3D_2_act =
        addElu(plugin_factory, *network, *conv3D_2_tran->getOutput(0), data_type, "conv3D_2_act");
    assert(conv3D_2_act != nullptr);
    conv3D_2_act->setName("conv3D_2_act");

    // conv3D_3ds_pad padding op.
    auto conv3D_3ds_pad = addPad(plugin_factory, *network, *conv3D_2_act->getOutput(0),
    ...
    auto conv3D_3ds = addConv3D(plugin_factory, *network, *conv3D_3ds_pad->getOutput(0),
                                ...
                                Dims{3, {0, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_3ds_k"),
    ...
    auto conv3D_4 = addConv3D(plugin_factory, *network, *conv3D_3ds_act->getOutput(0),
                              ...
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_4_k"),
                              weights.at("conv3D_4_b"), "conv3D_4");
    assert(conv3D_4 != nullptr);
    conv3D_4->setName("conv3D_4");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_4_tran = addTransform(plugin_factory, *network, *conv3D_4->getOutput(0),
                                      {1, 0, 2, 3}, "conv3D_4_tran_transform");
    assert(conv3D_4_tran != nullptr);
    conv3D_4_tran->setName("conv3D_4_tran");

    // conv3D_4_act ELU activation op.
    auto conv3D_4_act =
        addElu(plugin_factory, *network, *conv3D_4_tran->getOutput(0), data_type, "conv3D_4_act");
    ...
    auto conv3D_5 = addConv3D(plugin_factory, *network, *conv3D_4_act->getOutput(0),
                              Conv3DType::kTensorFlow, {5, {64, 3, 64, 3, 3}}, Dims{3, {1, 1, 1}},
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_5_k"),
                              weights.at("conv3D_5_b"), "conv3D_5");
    assert(conv3D_5 != nullptr);
    conv3D_5->setName("conv3D_5");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_5_tran = addTransform(plugin_factory, *network, *conv3D_5->getOutput(0),
                                      {1, 0, 2, 3}, "conv3D_5_tran_transform");
    assert(conv3D_5_tran != nullptr);
    conv3D_5_tran->setName("conv3D_5_tran");

    // conv3D_5_act ELU activation op.
    auto conv3D_5_act =
        addElu(plugin_factory, *network, *conv3D_5_tran->getOutput(0), data_type, "conv3D_5_act");
    assert(conv3D_5_act != nullptr);
    conv3D_5_act->setName("conv3D_5_act");

    // conv3D_6ds_pad padding op.
    auto conv3D_6ds_pad = addPad(plugin_factory, *network, *conv3D_5_act->getOutput(0),
                                 {0, 0, 0, 0}, {1, 0, 0, 0}, "conv3D_6ds_pad");
    assert(conv3D_6ds_pad != nullptr);
    conv3D_6ds_pad->setName("conv3D_6ds_pad");

    // conv3D_6ds 3D convolution op.
    auto conv3D_6ds = addConv3D(
        plugin_factory, *network, *conv3D_6ds_pad->getOutput(0), Conv3DType::kTensorFlow,
        {5, {128, 3, 64, 3, 3}}, Dims{3, {2, 2, 2}}, Dims{3, {0, 1, 1}}, Dims{3, {1, 1, 1}},
        weights.at("conv3D_6ds_k"), weights.at("conv3D_6ds_b"), "conv3D_6ds");
    assert(conv3D_6ds != nullptr);
    conv3D_6ds->setName("conv3D_6ds");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_6ds_tran = addTransform(plugin_factory, *network, *conv3D_6ds->getOutput(0),
                                        {1, 0, 2, 3}, "conv3D_6ds_tran_transform");
    assert(conv3D_6ds_tran != nullptr);
    conv3D_6ds_tran->setName("conv3D_6ds_tran");

    // conv3D_6ds_act ELU activation op.
    auto conv3D_6ds_act = addElu(plugin_factory, *network, *conv3D_6ds_tran->getOutput(0),
                                 data_type, "conv3D_6ds_act");
    ...
    auto conv3D_7 = addConv3D(plugin_factory, *network, *conv3D_6ds_act->getOutput(0),
                              Conv3DType::kTensorFlow, {5, {128, 3, 128, 3, 3}}, Dims{3, {1, 1, 1}},
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_7_k"),
                              weights.at("conv3D_7_b"), "conv3D_7");
    assert(conv3D_7 != nullptr);
    conv3D_7->setName("conv3D_7");

    // Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
    auto conv3D_7_tran = addTransform(plugin_factory, *network, *conv3D_7->getOutput(0),
                                      {1, 0, 2, 3}, "conv3D_7_tran_transform");
    ...
    auto conv3D_8 = addConv3D(plugin_factory, *network, *conv3D_7_act->getOutput(0),
                              Conv3DType::kTensorFlow, {5, {128, 3, 128, 3, 3}}, Dims{3, {1, 1, 1}},
                              Dims{3, {1, 1, 1}}, Dims{3, {1, 1, 1}}, weights.at("conv3D_8_k"),
                              weights.at("conv3D_8_b"), "conv3D_8");
    assert(conv3D_8 != nullptr);
    conv3D_8->setName("conv3D_8");

    // conv3D_8_act ELU activation op.
    auto conv3D_8_act =
        addElu(plugin_factory, *network, *conv3D_8->getOutput(0), data_type, "conv3D_8_act");
    ...
    auto deconv3D_1 = addConv3DTranspose(
        plugin_factory, *network, *conv3D_8_act->getOutput(0), Conv3DType::kTensorFlow,
        {5, {128, 3, 64, 3, 3}}, deconv3D_1_out_dims, Dims{3, {2, 2, 2}}, Dims{3, {0, 1, 1}},
        Dims{3, {0, 1, 1}}, weights.at("deconv3D_1_k"), weights.at("deconv3D_1_b"), "deconv3D_1");
    ...
    auto deconv3D_1_slice_layer = addSlice(plugin_factory, *network, *deconv3D_1->getOutput(0),
                                           deconv3D_1_out_dims, {4, {0, 0, 0, 0}},
                                           {4,
                                            {deconv3D_1_out_dims.d[0] - 1, deconv3D_1_out_dims.d[1],
                                             deconv3D_1_out_dims.d[2], deconv3D_1_out_dims.d[3]}},
                                           "deconv3D_1_slice");
    ...
    auto deconv3D_1_add_skip =
        network->addElementWise(*(deconv3D_1_slice_layer->getOutput(0)),
                                *(conv3D_5_act->getOutput(0)), ElementWiseOperation::kSUM);
    ...
    auto deconv3D_2 = addConv3DTranspose(
        ...
        {5, {64, 3, 32, 3, 3}}, deconv3D_2_out_dims, Dims{3, {2, 2, 2}}, Dims{3, {0, 1, 1}},
        Dims{3, {0, 1, 1}}, weights.at("deconv3D_2_k"), weights.at("deconv3D_2_b"), "deconv3D_2");
    ...
    auto deconv3D_2_slice_layer = addSlice(plugin_factory, *network, *deconv3D_2->getOutput(0),
                                           deconv3D_2_out_dims, {4, {0, 0, 0, 0}},
                                           {4,
                                            {deconv3D_2_out_dims.d[0] - 1, deconv3D_2_out_dims.d[1],
                                             deconv3D_2_out_dims.d[2], deconv3D_2_out_dims.d[3]}},
                                           "deconv3D_2_slice");
    ...
    auto deconv3D_2_add_skip =
        network->addElementWise(*(deconv3D_2_slice_layer->getOutput(0)),
                                *(conv3D_2_act->getOutput(0)), ElementWiseOperation::kSUM);
    ...
    auto deconv3D_3 = addConv3DTranspose(
        ...
        {5, {32, 3, 1, 3, 3}}, deconv3D_3_out_dims, Dims{3, {2, 2, 2}}, Dims{3, {0, 1, 1}},
    ...
    // Softargmax.
    auto disp = addSoftargmax(plugin_factory, *network, *deconv3D_3_slice_layer->getOutput(0),
