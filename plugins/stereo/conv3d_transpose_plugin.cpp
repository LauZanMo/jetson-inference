...
public:
    Conv3DTransposePlugin(Conv3DType  conv_type,
                          Dims        kernel_dims,
                          Dims        out_dims,
                          Dims        stride_dims,
                          Dims        pad_start_dims,
        ...
        : conv_type_(conv_type),
          w_dims_(kernel_dims),
          stride_dims_(stride_dims),
          pad_start_dims_(pad_start_dims),
          pad_end_dims_(pad_end_dims),
          kernel_weights_(kernel_weights),
          bias_weights_(bias_weights),
          log_(log),
          name_(name) {
        ...
        assert(pad_start_dims_.d[0] == pad_end_dims_.d[0] ||
               pad_start_dims_.d[0] == pad_end_dims_.d[0] - 1);
    ...
    Dims getOutputDimensions(int index, const Dims *inputs, int nbInputDims) override {
        ...
        ConvUtils::setConv3DTensorDescriptor(Conv3DType::kCuDnn, y_dims_, 1, weights_type_, y_desc_,
                                             log_);
        ...
        ConvUtils::setConv3DTensorDescriptor(Conv3DType::kCuDnn, x_dims_, 1, weights_type_,
                                             xo_desc_, log_);
        // Verify that dimensions match.
        std::array<int, 5> out_dims;
        CHECK(cudnnGetConvolutionNdForwardOutputDim(c_desc_, x_desc_, w_desc_, out_dims.size(),
                                                    out_dims.data()));
        assert(DimsUtils::areEqual(y_dims_,
                                   DimsNCHW{out_dims[1], out_dims[2], out_dims[3], out_dims[4]}));
        // Set bias descriptor.
    ...
    void configure(const Dims *inputDims,
                   int         nbInputs,
        ...
        ConvUtils::setConv3DTensorDescriptor(Conv3DType::kCuDnn, y_dims_, maxBatchSize,
                                             weights_type_, y_desc_, log_);
        ConvUtils::setConv3DTensorDescriptor(conv_type_, x_dims_, maxBatchSize, weights_type_,
                                             x_desc_, log_);
        ConvUtils::setConv3DTensorDescriptor(Conv3DType::kCuDnn, x_dims_, maxBatchSize,
                                             weights_type_, xo_desc_, log_);
        findBestAlgo();
        ...
        if (isFP16())
            workspace_bytes_ +=
                max_batch_size_ *
                std::max(DimsUtils::getTensorSize(x_dims_), DimsUtils::getTensorSize(y_dims_)) *
                elt_size;
        ...
        if (bias_weights_.count > 0) {
            ...
            CHECK(cudaMemcpy(bias_weights_d_, bias_weights_.values, bias_weights_.count * elt_size,
                             cudaMemcpyHostToDevice));
        }
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": InDims  : " + DimsUtils::toString(y_dims_)).c_str());
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": OutDims : " + DimsUtils::toString(x_dims_)).c_str());
    }
    ...
    int enqueue(int                batchSize,
        ...
        CHECK(status = cudnnConvolutionBackwardData(
                  cudnn_, &Consts::kOne, w_desc_, kernel_weights_d_, y_desc_, py, c_desc_,
                  best_algo_, static_cast<uint8_t *>(workspace) + workspace_used_bytes,
                  workspace_bytes_ - workspace_used_bytes, &Consts::kZero, x_desc_, outputs[0]));

        if (bias_weights_.count > 0) {
            // REVIEW alexeyk: cuDNN limitation - only second outermost dimension is supported.
            if (b_dims_.d[1] == x_dims_.d[0])
                CHECK(status = cudnnAddTensor(cudnn_, &Consts::kOne, b_desc_, bias_weights_d_,
                                              &Consts::kOne, x_desc_, outputs[0]));
            else if (b_dims_.d[2] == x_dims_.d[1])
                CHECK(cu_status = CudaKernels::addDBiasTo3DConv(bias_weights_d_, b_dims_,
...
private:
    ...
    const void *preprocessInput(int          batchSize,
                                const void  *y,
                                void        *workspace,
                                cudaStream_t stream,
    ...
    void postprocessOutput(int batchSize, void *x, void *workspace, cudaStream_t stream) {
        ...
        CHECK(cudaMemcpyAsync(workspace, x, x_size * sizeof(uint16_t), cudaMemcpyDeviceToDevice,
                              stream));
    ...
    void findBestAlgo() {
        // Let's hope cuDNN team will not come up with more than that number of algos (7 in cuDNN
        // 7).
        ...
        CHECK(cudnnGetConvolutionBackwardDataAlgorithm_v7(
            cudnn_, w_desc_, y_desc_, c_desc_, x_desc_, algo_count, &res_algo_count, algos));
#endif
        ...
        // Log results.
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": --> Conv3DTranspose layer tuning results:").c_str());
        for (auto &a : algos) {
            if (a.status != CUDNN_STATUS_SUCCESS)
                break;
            std::ostringstream str;
            str << a.algo << ": " << std::fixed << std::setw(8) << std::setprecision(1) << a.time
                << "ms, " << std::fixed << std::setw(8) << a.memory << "B";
...
// Factory method.
IPlugin *PluginContainer::createConv3DTransposePlugin(Conv3DType  conv_type,
                                                      Dims        kernel_dims,
                                                      Dims        out_dims,
                                                      Dims        stride_dims,
                                                      Dims        pad_start_dims,
    ...
    plugins_.push_back(new Conv3DTransposePlugin(conv_type, kernel_dims, out_dims, stride_dims,
                                                 pad_start_dims, pad_end_dims, kernel_weights,
                                                 bias_weights, log_, name));
