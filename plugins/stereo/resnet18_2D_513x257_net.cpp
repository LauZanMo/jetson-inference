...
INetworkDefinition *createResNet18_2D_513x257Network(IBuilder         &builder,
    ...
    auto cost_vol = addCostVolume(plugin_factory, *network, *left_encoder2D_out->getOutput(0),
                                  *right_encoder2D_out->getOutput(0), CostVolumeType::kCorrelation,
                                  48, data_type, "cost_vol");
    assert(cost_vol != nullptr);
    cost_vol->setName("cost_vol");

    // Softargmax.
    auto softargmax = addSoftargmax(plugin_factory, *network, *cost_vol->getOutput(0),
                                    SoftargmaxType::kMax, data_type, "softargmax_softargmax");
    ...
    auto conv2D_1_act =
        addElu(plugin_factory, *network, *conv2D_1->getOutput(0), data_type, "conv2D_1_act");
    ...
    auto conv2D_2_act =
        addElu(plugin_factory, *network, *conv2D_2->getOutput(0), data_type, "conv2D_2_act");
    assert(conv2D_2_act != nullptr);
    conv2D_2_act->setName("conv2D_2_act");

    // conv2D_3ds convolution op.
    auto conv2D_3ds =
        network->addConvolution(*conv2D_2_act->getOutput(0), 64, DimsHW{3, 3},
    ...
    auto conv2D_3ds_act =
        addElu(plugin_factory, *network, *conv2D_3ds->getOutput(0), data_type, "conv2D_3ds_act");
    ...
    auto conv2D_4_act =
        addElu(plugin_factory, *network, *conv2D_4->getOutput(0), data_type, "conv2D_4_act");
    ...
    auto conv2D_5_act =
        addElu(plugin_factory, *network, *conv2D_5->getOutput(0), data_type, "conv2D_5_act");
    assert(conv2D_5_act != nullptr);
    conv2D_5_act->setName("conv2D_5_act");

    // conv2D_6ds convolution op.
    auto conv2D_6ds =
        network->addConvolution(*conv2D_5_act->getOutput(0), 128, DimsHW{3, 3},
    ...
    auto conv2D_6ds_act =
        addElu(plugin_factory, *network, *conv2D_6ds->getOutput(0), data_type, "conv2D_6ds_act");
    ...
    auto conv2D_7_act =
        addElu(plugin_factory, *network, *conv2D_7->getOutput(0), data_type, "conv2D_7_act");
    ...
    auto conv2D_8_act =
        addElu(plugin_factory, *network, *conv2D_8->getOutput(0), data_type, "conv2D_8_act");
    assert(conv2D_8_act != nullptr);
    conv2D_8_act->setName("conv2D_8_act");

    // deconv2D_1 transposed convolution op.
    auto deconv2D_1 =
        network->addDeconvolution(*conv2D_8_act->getOutput(0), 64, DimsHW{3, 3},
    ...
    auto deconv2D_1_add_skip = network->addElementWise(
        *(deconv2D_1->getOutput(0)), *(conv2D_5_act->getOutput(0)), ElementWiseOperation::kSUM);
    assert(deconv2D_1_add_skip != nullptr);
    deconv2D_1_add_skip->setName("deconv2D_1_add_skip");

    // deconv2D_1_act ELU activation op.
    auto deconv2D_1_act = addElu(plugin_factory, *network, *deconv2D_1_add_skip->getOutput(0),
                                 data_type, "deconv2D_1_act");
    assert(deconv2D_1_act != nullptr);
    deconv2D_1_act->setName("deconv2D_1_act");

    // deconv2D_2 transposed convolution op.
    auto deconv2D_2 =
        network->addDeconvolution(*deconv2D_1_act->getOutput(0), 32, DimsHW{3, 3},
    ...
    auto deconv2D_2_add_skip = network->addElementWise(
        *(deconv2D_2->getOutput(0)), *(conv2D_2_act->getOutput(0)), ElementWiseOperation::kSUM);
    assert(deconv2D_2_add_skip != nullptr);
    deconv2D_2_add_skip->setName("deconv2D_2_add_skip");

    // deconv2D_2_act ELU activation op.
    auto deconv2D_2_act = addElu(plugin_factory, *network, *deconv2D_2_add_skip->getOutput(0),
                                 data_type, "deconv2D_2_act");
    assert(deconv2D_2_act != nullptr);
    deconv2D_2_act->setName("deconv2D_2_act");

    // deconv2D_3 transposed convolution op.
    auto deconv2D_3 =
        network->addDeconvolution(*deconv2D_2_act->getOutput(0), 1, DimsHW{3, 3},
