...
public:
    template <typename T>
    static cudaError_t computeCostVolume(DataType     data_type,
                                         const T     *left,
                                         const T     *right,
                                         Dims         in_dims,
                                         T           *cost_vol,
    ...
    template <typename T>
    static cudaError_t computeCorrCostVolume(DataType     data_type,
                                             const T     *left,
                                             const T     *right,
                                             Dims         in_dims,
                                             T           *cost_vol,
    ...
    template <typename T>
    static cudaError_t
    addDBiasTo3DConv(const T *bias, Dims bias_dims, T *conv, Dims conv_dims, cudaStream_t stream);

    static cudaError_t
    fp32Tofp16(const float *src, uint16_t *dst, size_t size, cudaStream_t stream);
    static cudaError_t
    fp16Tofp32(const uint16_t *src, float *dst, size_t size, cudaStream_t stream);
...
template <>
cudaError_t CudaKernels::computeCostVolume(
    DataType data_type, const float *, const float *, Dims, float *, Dims, cudaStream_t);
...
public:
    PluginContainer(ILogger &log) : log_(log) {}
    ...
    // Cost volume plugin.
    IPlugin *createCostVolumePlugin(DataType       data_type,
    ...
    // 3D convolution.
    IPlugin *createConv3DPlugin(Conv3DType  conv_type,
                                Dims        kernel_dims,
                                Dims        stride_dims,
                                Dims        pad_start_dims,
                                Dims        pad_end_dims,
    ...
    IPlugin *createConv3DTransposePlugin(Conv3DType  conv_type,
                                         Dims        kernel_dims,
                                         Dims        out_dims,
                                         Dims        stride_dims,
                                         Dims        pad_start_dims,
    ...
    // Padding.
    IPlugin *createPaddingPlugin(DimsNCHW pad_start, DimsNCHW pad_end, std::string name) override;

    IPlugin *
    createSlicePlugin(Dims dims, Dims slice_start, Dims slice_end, std::string name) override;

    IPlugin *
    createSoftargmaxPlugin(DataType data_type, SoftargmaxType sm_type, std::string name) override;
