...
public:
    Conv3DPlugin(Conv3DType  conv_type,
                 Dims        kernel_dims,
                 Dims        stride_dims,
                 Dims        pad_start_dims,
                 Dims        pad_end_dims,
    ...
    Dims getOutputDimensions(int index, const Dims *inputs, int nbInputDims) override {
        ...
        ConvUtils::setConv3DTensorDescriptor(Conv3DType::kCuDnn, y_dims_, 1, weights_type_, y_desc_,
                                             log_);
        // Set bias descriptor.
        // REVIEW alexeyk: see the comment in tensorrt_model_builder.py re: the stride issue in
        // Conv3D.
        ConvUtils::setConv3DBiasDescriptor(Dims{5, {1, y_dims_.d[0], 1, 1, 1}}, weights_type_,
                                           b_desc_, log_);
    ...
    void configure(const Dims *inputDims,
        ...
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": InDims  : " + DimsUtils::toString(x_dims_)).c_str());
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": OutDims : " + DimsUtils::toString(y_dims_)).c_str());
    }
    ...
    int enqueue(int                batchSize,
        ...
        CHECK(status = cudnnConvolutionForward(
                  cudnn_, &Consts::kOne, x_desc_, px, w_desc_, kernel_weights_d_, c_desc_,
                  best_algo_, static_cast<uint8_t *>(workspace) + workspace_used_bytes,
                  workspace_bytes_ - workspace_used_bytes, &Consts::kZero, y_desc_, outputs[0]));

        if (bias_weights_.count > 0)
            CHECK(status = cudnnAddTensor(cudnn_, &Consts::kOne, b_desc_, bias_weights_d_,
                                          &Consts::kOne, y_desc_, outputs[0]));
...
private:
    ...
    void postprocessOutput(int batchSize, void *y, void *workspace, cudaStream_t stream) {
        ...
        CHECK(cudaMemcpyAsync(workspace, y, y_size * sizeof(uint16_t), cudaMemcpyDeviceToDevice,
                              stream));
    ...
    void findBestAlgo() {
        // Let's hope cuDNN team will not come up with more than that number of algos (8 in cuDNN
        // 7).
...
// Factory method.
IPlugin *PluginContainer::createConv3DPlugin(Conv3DType  conv_type,
                                             Dims        kernel_dims,
                                             Dims        stride_dims,
                                             Dims        pad_start_dims,
                                             Dims        pad_end_dims,
    ...
    plugins_.push_back(new Conv3DPlugin(conv_type, kernel_dims, stride_dims, pad_start_dims,
                                        pad_end_dims, kernel_weights, bias_weights, log_, name));
