...
public:
    CostVolumePlugin(DataType       data_type,
                     CostVolumeType cv_type,
                     int            max_disparity,
                     ILogger       &log,
                     std::string    name)
        : data_type_(data_type),
          format_(PluginFormat::kNCHW),
          cv_type_(cv_type),
    ...
    bool supportsFormat(DataType type, PluginFormat format) const override {
        ...
        // REVIEW alexeyk: kHALF && kNCHW is only for testing on the host as TRT fails with assert
        // when using kHALF && kNC2HW2.
    ...
    void configureWithFormat(const Dims  *inputDims,
        ...
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": InDims(x2): " + DimsUtils::toString(in_dims_)).c_str());
        log_.log(ILogger::Severity::kINFO,
    ...
    int enqueue(int                batchSize,
        ...
        if (cv_type_ == CostVolumeType::kDefault)
            CHECK(status = CudaKernels::computeCostVolume(data_type_, pleft, pright, in_dims_,
                                                          pcost_vol, out_dims_, stream));
        else if (cv_type_ == CostVolumeType::kCorrelation)
            CHECK(status = CudaKernels::computeCorrCostVolume(data_type_, pleft, pright, in_dims_,
                                                              pcost_vol, out_dims_, stream));
        else
            assert(false);
...
// Factory method.
IPlugin *PluginContainer::createCostVolumePlugin(DataType       data_type,
                                                 CostVolumeType cv_type,
                                                 int            max_disparity,
                                                 std::string    name) {
...
IPlugin *
PluginContainer::deserializeCostVolumePlugin(const char *name, const void *data, size_t size) {
    std::lock_guard<std::mutex> lock(lock_);
