...
public:
    SlicePlugin(Dims dims, Dims slice_start, Dims slice_end, ILogger &log, std::string name)
        : in_dims_(dims), slice_start_(slice_start), slice_end_(slice_end), log_(log), name_(name) {
    ...
    Dims getOutputDimensions(int index, const Dims *inputs, int nbInputDims) override {
        ...
        out_dims_ = DimsNCHW(slice_end_.d[0] - slice_start_.d[0], inputs[0].d[1], inputs[0].d[2],
                             inputs[0].d[3]);
        return out_dims_;
    }
    ...
    int enqueue(int                batchSize,
        ...
        cudaError_t status;
        CHECK(status = cudaMemcpyAsync(outputs[0], psrc, size * sizeof(float),
...
// Factory method.
IPlugin *
PluginContainer::createSlicePlugin(Dims dims, Dims slice_start, Dims slice_end, std::string name) {
