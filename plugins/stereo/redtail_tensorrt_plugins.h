...
public:
    ...
    // Cost volume plugin.
    virtual IPlugin *createCostVolumePlugin(DataType       data_type,
                                            CostVolumeType cv_type,
                                            int            max_disparity,
                                            std::string    name) = 0;
    virtual IPlugin *
    deserializeCostVolumePlugin(const char *name, const void *data, size_t size) = 0;

    // 3D convolution.
    virtual IPlugin *createConv3DPlugin(Conv3DType  conv_type,
                                        Dims        kernel_dims,
                                        Dims        stride_dims,
                                        Dims        pad_start_dims,
                                        Dims        pad_end_dims,
    ...
    virtual IPlugin *createConv3DTransposePlugin(Conv3DType  conv_type,
                                                 Dims        kernel_dims,
                                                 Dims        out_dims,
                                                 Dims        stride_dims,
                                                 Dims        pad_start_dims,
    ...
    virtual IPlugin *
    createPaddingPlugin(DimsNCHW pad_start, DimsNCHW pad_end, std::string name) = 0;

    virtual IPlugin *
    createSlicePlugin(Dims dims, Dims slice_start, Dims slice_end, std::string name) = 0;

    virtual IPlugin *
    createSoftargmaxPlugin(DataType data_type, SoftargmaxType sm_type, std::string name) = 0;
    virtual IPlugin *
    deserializeSoftargmaxPlugin(const char *name, const void *data, size_t size) = 0;
