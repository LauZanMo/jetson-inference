...
public:
    ...
    void configureWithFormat(const Dims  *inputDims,
        ...
        auto str = name_ + ": Dims: " + DimsUtils::toString(in_dims_) + ", Format: [" +
                   StrUtils::toString(type) + ", " + StrUtils::toString(format) + "]";
    ...
    int initialize() override {
        ...
        CHECK(status = cudnnSetActivationDescriptor(act_, CUDNN_ACTIVATION_ELU, CUDNN_PROPAGATE_NAN,
                                                    1.0));
    ...
    int enqueue(int                batchSize,
        ...
        CHECK(status = cudnnActivationForward(cudnn_, act_, &Consts::kOne, t_desc_, inputs[0],
                                              &Consts::kZero, t_desc_, outputs[0]));
...
private:
    ...
    void setTensorDescriptor() {
        ...
        CHECK(cudnnSetTensorNdDescriptor(t_desc_, trtToCudnnDataType(data_type_),
                                         tensor_dims_.nbDims, tensor_dims_.d, tensor_strides_.d));
    }
