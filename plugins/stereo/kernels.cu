...
template <typename T>
__global__ void
costVolumeCopyKernel(const T *src, int32_t c, int32_t h, int32_t w, int32_t disp, T *dst) {
    assert(src != nullptr);
    assert(dst != nullptr);
...
template <typename T>
__global__ void
costVolumeCopyPadKernel(const T *src, int32_t c, int32_t h, int32_t w, int32_t disp, T *dst) {
    assert(src != nullptr);
    assert(dst != nullptr);
...
template <typename T>
__global__ void costVolumeKernel(
    const T *left, const T *right, int32_t c, int32_t h, int32_t w, int32_t disp, T *dst) {
    assert(left != nullptr);
...
template <>
cudaError_t CudaKernels::computeCostVolume(DataType     data_type,
                                           const float *left,
                                           const float *right,
                                           Dims         in_dims,
                                           float       *cost_vol,
    ...
    // REVIEW alexeyk: using 2 kernels instead of one as it's not yet optimized so 2 kernels are
    // faster. REVIEW alexeyk: optimize, see
    // gld_efficiency,gst_efficiency,gld_transactions,gst_transactions. costVolumeKernel<<<g_dim,
    // b_dim, 0, stream>>>(left, right, in_dims.d[0], in_dims.d[1], in_dims.d[2], out_dims.d[0],
    //                                               cost_vol);
    costVolumeCopyKernel<<<g_dim, b_dim, 0, stream>>>(left, in_dims.d[0], in_dims.d[1],
                                                      in_dims.d[2], out_dims.d[0], cost_vol);
    CHECKK(stream);
    costVolumeCopyPadKernel<<<g_dim, b_dim, 0, stream>>>(right, in_dims.d[0], in_dims.d[1],
                                                         in_dims.d[2], out_dims.d[0], cost_vol);
    CHECKK(stream);
    return cudaSuccess;
}
...
// FP32, NCHW kernel.
template <typename T>
__global__ void corrCostVolumeKernel(
    const T *left, const T *right, int32_t c, int32_t h, int32_t w, int32_t disp, T *dst) {
    assert(left != nullptr);
    assert(right != nullptr);
    assert(dst != nullptr);
...
// FP16, NC2HW2 kernel.
__global__ void corrCostVolumeFP16NC2HW2Kernel(const float *left,
                                               const float *right,
                                               int32_t      c,
                                               int32_t      h,
                                               int32_t      w,
...
template <>
cudaError_t CudaKernels::computeCorrCostVolume(DataType     data_type,
                                               const float *left,
                                               const float *right,
                                               Dims         in_dims,
                                               float       *cost_vol,
    ...
    if (data_type == DataType::kFLOAT) {
        ...
        corrCostVolumeKernel<<<g_dim, b_dim, 0, stream>>>(left, right, in_dims.d[0], in_dims.d[1],
                                                          in_dims.d[2], out_dims.d[0], cost_vol);
        CHECKK(stream);
        ...
        corrCostVolumeFP16NC2HW2Kernel<<<g_dim, b_dim, 0, stream>>>(
            left, right, in_dims.d[0], in_dims.d[1], in_dims.d[2], out_dims.d[0], cost_vol);
        CHECKK(stream);
    }
    return cudaSuccess;
}
...
template <typename T>
__global__ void
addDBiasTo3DConvKernel(const T *bias, int32_t c, int32_t d, int32_t h, int32_t w, T *conv) {
    assert(bias != nullptr);
    assert(conv != nullptr);
...
template <>
cudaError_t CudaKernels::addDBiasTo3DConv(
    const float *bias, Dims bias_dims, float *conv, Dims conv_dims, cudaStream_t stream) {
    ...
    addDBiasTo3DConvKernel<<<g_dim, b_dim, 0, stream>>>(bias, conv_dims.d[0], conv_dims.d[1],
                                                        conv_dims.d[2], conv_dims.d[3], conv);
    CHECKK(stream);
...
cudaError_t
CudaKernels::fp32Tofp16(const float *src, uint16_t *dst, size_t size, cudaStream_t stream) {
    dim3 b_dim{256, 1, 1};
    dim3 g_dim;
...
cudaError_t
CudaKernels::fp16Tofp32(const uint16_t *src, float *dst, size_t size, cudaStream_t stream) {
    dim3 b_dim{256, 1, 1};
    dim3 g_dim;
    g_dim.x = getBlockCount(size, b_dim.x);
