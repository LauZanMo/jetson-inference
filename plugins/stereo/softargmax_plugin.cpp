...
public:
    ...
    Dims getOutputDimensions(int index, const Dims *inputs, int nbInputDims) override {
        ...
        // If input is 5D tensor in NDCHW format (batch index implicit) then C dim must be equal
        // to 1.
    ...
    void configureWithFormat(const Dims  *inputDims,
                             int          nbInputs,
        ...
        if (inputDims[0].nbDims == 3)
            ...
            assert(DimsUtils::areEqual(inputDims[0],
                                       Dims{4, {in_dims_.d[0], 1, in_dims_.d[1], in_dims_.d[2]}}));
        ...
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": InDims : " + DimsUtils::toString(in_dims_)).c_str());
        log_.log(ILogger::Severity::kINFO,
                 (name_ + ": OutDims: " + DimsUtils::toString(out_dims_)).c_str());
    ...
    int enqueue(int                batchSize,
        ...
        CHECK(cu_status = cudaMemcpyAsync(pdst, inputs[0], in_size_bytes, cudaMemcpyDeviceToDevice,
                                          stream));
        ...
        CHECK(status = cudnnSoftmaxForward(cudnn_, CUDNN_SOFTMAX_ACCURATE,
                                           CUDNN_SOFTMAX_MODE_CHANNEL, &Consts::kOne, in_desc_,
                                           pdst, &Consts::kZero, in_desc_, pdst));

        // Multiply and sum-reduce over D dim.
        CHECK(status = cudnnOpTensor(cudnn_, op_desc_, &Consts::kOne, in_desc_, pdst, &Consts::kOne,
                                     idx_desc_, indices_d_, &Consts::kZero, in_desc_, pdst));
        CHECK(status = cudnnReduceTensor(cudnn_, sum_desc_, nullptr, 0, pdst + in_size,
                                         in_size_bytes, &Consts::kOne, in_desc_, pdst,
...
private:
    ...
    void setTensorDescriptors(int batch_size) {
        ...
        CHECK(cudnnSetTensorNdDescriptor(in_desc_, cu_data_type, tensor_dims.nbDims, tensor_dims.d,
                                         tensor_strides.d));
        ...
        CHECK(cudnnSetTensorNdDescriptor(out_desc_, cu_data_type, tensor_dims.nbDims, tensor_dims.d,
                                         tensor_strides.d));

        // Set index tensor.
        tensor_dims    = Dims{4, {1, in_dims_.d[0], 1, 1}};
        tensor_strides = DimsUtils::getStrides(tensor_dims);
        CHECK(cudnnSetTensorNdDescriptor(idx_desc_, cu_data_type, tensor_dims.nbDims, tensor_dims.d,
                                         tensor_strides.d));
        ...
        CHECK(cudnnSetOpTensorDescriptor(op_desc_, CUDNN_OP_TENSOR_MUL, CUDNN_DATA_FLOAT,
                                         CUDNN_PROPAGATE_NAN));

        // Set reduce descriptor.
        CHECK(cudnnSetReduceTensorDescriptor(sum_desc_, CUDNN_REDUCE_TENSOR_ADD, CUDNN_DATA_FLOAT,
...
// Factory method.
IPlugin *PluginContainer::createSoftargmaxPlugin(DataType       data_type,
                                                 SoftargmaxType sm_type,
                                                 std::string    name) {
...
IPlugin *
PluginContainer::deserializeSoftargmaxPlugin(const char *name, const void *data, size_t size) {
    std::lock_guard<std::mutex> lock(lock_);
