...
public:
    ...
    // -----------------------------------------------------------------
    static void setConv3DTensorDescriptor(Conv3DType               conv_type,
                                          Dims                     dims,
                                          int                      batch_size,
    ...
    //
    // w_dims:
    //   Filter dimensions, must have rank 5. The filter tensor is assumed to be in (K, C, V, R, S)
    //   format.
    // pad_dims:
    //   Input padding dimensions, must have rank 3. The padding is for (Di, Hi, Wi) dimensions of
    //   the convolution input (transposed conovlution output).
    // stride_dims:
    //   Filter stride dimensions, must have rank 3. The stride is for (Di, Hi, Wi) dimensions of
    //   the convolution input (transposed conovlution output).
    // -----------------------------------------------------------------
    static void setConv3DOperationDescriptors(Conv3DType                    conv_type,
                                              Dims                          w_dims,
                                              Dims                          stride_dims,
                                              Dims                          pad_dims,
                                              cudnnDataType_t               data_type,
    ...
    // -----------------------------------------------------------------
    static Dims getConv3DOutputDims(cudnnConvolutionDescriptor_t c_desc,
                                    cudnnTensorDescriptor_t      x_desc,
                                    cudnnFilterDescriptor_t      w_desc,
                                    ILogger                     &log);

    static void setConv3DBiasDescriptor(Dims                     dims,
                                        cudnnDataType_t          data_type,
                                        cudnnTensorDescriptor_t &desc,
                                        ILogger                 &log);
