...
int StridedSlicePlugin::enqueue(int                batchSize,
    ...
    if (mDataType == DataType::kFLOAT)
        CHECK(cudaSlice<float>((float *)inputs[0], SLICE_INPUT_C, SLICE_INPUT_H, SLICE_INPUT_W,
                               (float *)outputs[0], stream));
    else
        CHECK(cudaSlice<__half>((__half *)inputs[0], SLICE_INPUT_C, SLICE_INPUT_H, SLICE_INPUT_W,
                                (__half *)outputs[0], stream));
    return 0;
}

IPluginV2 *StridedSlicePluginCreator::createPlugin(const char                  *name,
