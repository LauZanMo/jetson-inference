...
template <typename T>
__global__ void
gpuSlice(T *input, int nChannels, int iHeight, int iWidth, int oHeight, int oWidth, T *output) {
...
cudaError_t cudaSlice(
    ...
    gpuSlice<T><<<gridDim, blockDim, 0, stream>>>(input, nChannels, inputHeight, inputWidth,
                                                  outputHeight, outputWidth, output);
