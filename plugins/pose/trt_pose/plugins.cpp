...
void find_peaks_out_torch(torch::Tensor counts,
                          torch::Tensor peaks,
                          torch::Tensor input,
    ...
    // find peaks
    find_peaks_out_nchw(counts_ptr, peaks_ptr, input_ptr, N, C, H, W, M, threshold, window_size);
}
...
void refine_peaks_out_torch(torch::Tensor refined_peaks,
                            torch::Tensor counts,
                            torch::Tensor peaks,
                            torch::Tensor cmap,
                            int           window_size) {
    ...
    refine_peaks_out_nchw((float *)refined_peaks.data_ptr(), (const int *)counts.data_ptr(),
                          (const int *)peaks.data_ptr(), (const float *)cmap.data_ptr(), N, C, H, W,
                          M, window_size);
}

torch::Tensor
refine_peaks_torch(torch::Tensor counts, torch::Tensor peaks, torch::Tensor cmap, int window_size) {
    auto options = torch::TensorOptions()
    ...
    auto refined_peaks =
        torch::zeros({peaks.size(0), peaks.size(1), peaks.size(2), peaks.size(3)}, options);
    refine_peaks_out_torch(refined_peaks, counts, peaks, cmap, window_size);
    return refined_peaks;
}

void paf_score_graph_out_torch(torch::Tensor score_graph,
                               torch::Tensor paf,
    ...
    paf_score_graph_out_nkhw((float *)score_graph.data_ptr(), (const int *)topology.data_ptr(),
                             (const float *)paf.data_ptr(), (const int *)counts.data_ptr(),
                             (const float *)peaks.data_ptr(), N, K, C, H, W, M,
                             num_integral_samples);
}

torch::Tensor paf_score_graph_torch(torch::Tensor paf,
                                    torch::Tensor topology,
                                    torch::Tensor counts,
                                    torch::Tensor peaks,
                                    const int     num_integral_samples) {
...
void assignment_out_torch(torch::Tensor connections,
                          torch::Tensor score_graph,
                          torch::Tensor topology,
                          torch::Tensor counts,
    ...
    assignment_out_nk((int *)connections.data_ptr(), (const float *)score_graph.data_ptr(),
...
torch::Tensor assignment_torch(torch::Tensor score_graph,
    ...
    assignment_out_torch(connections, score_graph, topology, counts, score_threshold);
    return connections;
}
...
void connect_parts_out_torch(torch::Tensor object_counts,
    ...
    connect_parts_out_batch((int *)object_counts.data_ptr(), (int *)objects.data_ptr(),
                            (const int *)connections.data_ptr(), (const int *)topology.data_ptr(),
                            (const int *)counts.data_ptr(), N, K, C, M, P, workspace);
    free(workspace);
}
