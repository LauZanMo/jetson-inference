...
void paf_score_graph_out_khw(float       *score_graph, // KxMxM
    ...
    for (int k = 0; k < K; k++) {
        ...
        paf_score_graph_out_hw(score_graph_k, paf_i, paf_j, counts_a, counts_b, peaks_a, peaks_b, H,
                               W, M, num_integral_samples);
    }
}
...
void paf_score_graph_out_nkhw(float       *score_graph, // NxKxMxM
                              ...
                              const int    num_integral_samples) {
    for (int n = 0; n < N; n++) {
        paf_score_graph_out_khw(&score_graph[n * K * M * M], topology, &paf[n * 2 * K * H * W],
                                &counts[n * C], &peaks[n * C * M * 2], K, C, H, W, M,
