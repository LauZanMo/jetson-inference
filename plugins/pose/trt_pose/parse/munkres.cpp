...
void munkresStep1(
    const float *cost_graph, const int M, PairGraph &star_graph, const int nrows, const int ncols) {
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
...
bool munkresStep3(const float         *cost_graph,
                  const int            M,
                  const PairGraph     &star_graph,
                  PairGraph           &prime_graph,
                  CoverTable          &cover_table,
                  std::pair<int, int> &p,
                  const int            nrows,
                  const int            ncols) {
    for (int i = 0; i < nrows; i++) {
...
void munkresStep4(PairGraph          &star_graph,
                  PairGraph          &prime_graph,
                  CoverTable         &cover_table,
                  std::pair<int, int> p) {
...
void munkresStep5(float            *cost_graph,
                  const int         M,
                  const CoverTable &cover_table,
                  const int         nrows,
                  const int         ncols) {
    bool  valid = false;
    float min;
...
void _munkres(
    float *cost_graph, const int M, PairGraph &star_graph, const int nrows, const int ncols) {
    ...
    while (!done) {
        ...
        case 3:
            if (!munkresStep3(cost_graph, M, star_graph, prime_graph, cover_table, p, nrows,
                              ncols)) {
                step = 5;
                break;
            }
...
void assignment_out_k(int         *connections, // Kx2xM
    ...
    for (int k = 0; k < K; k++) {
        ...
        assignment_out(&connections[k * 2 * M], &score_graph[k * M * M], count_a, count_b, M,
...
void assignment_out_nk(int         *connections, // NxKx2xM
                       ...
                       void        *workspace) {
    for (int n = 0; n < N; n++) {
        assignment_out_k(&connections[n * K * 2 * M], &score_graph[n * K * M * M], topology,
                         &counts[n * C], K, M, score_threshold, workspace);
    }
}
