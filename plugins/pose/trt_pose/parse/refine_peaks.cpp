...
void refine_peaks_out_chw(float       *refined_peaks, // CxMx2
                          ...
                          const int    window_size) {
    for (int c = 0; c < C; c++) {
        refine_peaks_out_hw(&refined_peaks[c * M * 2], &counts[c], &peaks[c * M * 2],
                            &cmap[c * H * W], H, W, M, window_size);
    }
}
...
void refine_peaks_out_nchw(float       *refined_peaks, // NxCxMx2
                           ...
                           const int    window_size) {
    for (int n = 0; n < N; n++) {
        refine_peaks_out_chw(&refined_peaks[n * C * M * 2], &counts[n * C], &peaks[n * C * M * 2],
                             &cmap[n * C * H * W], C, H, W, M, window_size);
    }
}
