...
int InterleavingPlugin::enqueue(int                batchSize,
    ...
    if (mDataType == DataType::kFLOAT)
        CHECK(cudaInterleave<float>((float *)inputs[0], (float *)inputs[1], (float *)inputs[2],
                                    (float *)inputs[3], mNbInputChannels, mInputHeight, mInputWidth,
                                    (float *)outputs[0], stream));
    else
        CHECK(cudaInterleave<__half>((__half *)inputs[0], (__half *)inputs[1], (__half *)inputs[2],
                                     (__half *)inputs[3], mNbInputChannels, mInputHeight,
                                     mInputWidth, (__half *)outputs[0], stream));
    return 0;
}

IPluginV2 *InterleavingPluginCreator::createPlugin(const char                  *name,
